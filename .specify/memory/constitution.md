...
4. **ブランチ運用**: `/speckit-plan`・`/speckit-tasks` はそれぞれ専用ブランチ
   ...
   ただし、次の条件をすべて満たす小さな変更は、`/speckit-specify` から `/speckit-implement`
   までを 1 つのブランチ・1 つの PR で進めてよい:
   - 既存の spec への FR の追加・改訂が 1〜2 件程度にとどまる
   - 新しい画面や、設定ファイルのデータ構造の変更を伴わない
   - 実装が 1 つのユーザーストーリーの中で完結する

   この場合も、各フェーズの成果物 (`spec.md`・`plan.md`・`tasks.md`) はフェーズごとに
   コミットを分け、どの段階で何を決めたかを追えるようにする。小さな変更として扱うかどうかは、
   `/speckit-specify` の時点で Claude が提案し、人間が決める
...
6. **レビューとマージのゲート**: PR は人間が GitHub 上でレビューし、マージを判断する。Claude は
   ...
   ブランチを作成する
7. **UI フレームワークの挙動の事前確認**: `/speckit-plan` で WPF の挙動に依存する設計をする
   場合は、前提としている挙動を `plan.md` (または `research.md`) に列挙し、最小のアプリで
   実際の挙動を確かめてから設計を確定する。確かめた方法と結果もあわせて記録し、確かめられ
   なかった前提は「未確認」と明記する。Constitution Check ではこの一覧に未確認の前提が
   残っていないかを確認する。対象となる挙動の例 (いずれも確かめずに設計して外した実例):
   - イベントの発火順序と、その時点での状態 (`SizeChanged` の時点では Win32 側のウィンドウが
     まだ変化前のサイズ。issue #36)
   - テーマによる描かれ方の違い (Fluent テーマの `MenuItem` は `IsCheckable` がないと
     チェックを描かない。issue #34)
   - 無効化・非表示にしたときの子要素の挙動 (無効にした親メニューのサブメニューは開けない。
     アンカー配置の `/speckit-analyze` 指摘 I1)

   手動の確認手順 (`quickstart.md`) は人間の実施に依存し、見落としを防ぎきれない。
   フレームワークの挙動に起因する不具合は、設計の段階で防ぐことを優先する
...
**Version**: 1.5.0 | **Ratified**: 2026-09-17 | **Last Amended**: 2026-09-25
